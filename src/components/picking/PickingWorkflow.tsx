...
export const PickingWorkflow = () => {
  const [selectedOrder, setSelectedOrder] = useState<number | null>(null);

  const { data: orders, isLoading } = useQuery({
    queryKey: ["packing-lists", "to-pick"],
    queryFn: async () => {
  ...
  const handleScan = async (barcode: string) => {
    ...
    // Update the picked status in packing_list_items
    ...
    if (error) {
      toast.error("Failed to update pick status");
      return;
    }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <h3 className="text-lg font-semibold">Select Order to Pick</h3>
          {orders?.map((order) => (
            <Button
              key={order.id}
